...
namespace MortgageAPI.Repos.Helper
{
    public class AmortizationCalculator : IAmortizationCalculator
    {
        ...
        private List<AmortizationSchedule> GenerateFixedRateSchedule(Loan loan)
        {
            var (rateDate, baseRate, _) = _rateProvider.GetClosestFreddieRate(
                loan.LoanProduct.TermYears,
                loan.ApplicationDate
            );

            // Add margin (if any) here instead of inside RateProvider
            loan.InterestRate = baseRate + (loan.LoanProduct.InitialRateMargin ?? 0m);
        ...
        private List<AmortizationSchedule> GenerateARMSchedule(Loan loan)
        {
            var baseRates = _rateProvider.GetRawSofrRatesUpTo(loan.ApplicationDate);
            ...
            decimal initialRate = baseRates.First().rate + (loan.LoanProduct.InitialRateMargin ?? 0m);

            // Fixed-rate intro period
            ...
            {
                decimal adjustedRate = rateIndex < baseRates.Count
                    ? baseRates[rateIndex].rate + (loan.LoanProduct.InitialRateMargin ?? 0m)
                    : rates.Last();
            ...
            return GenerateArmScheduleFromRates(loan, rates);
        }
        ...
        private List<AmortizationSchedule> GenerateArmScheduleFromRates(Loan loan, List<decimal> rates)
        {
            var schedule = new List<AmortizationSchedule>();
            decimal balance = loan.LoanAmount;
            DateTime paymentDate = loan.ApplicationDate;
            ...
            for (int i = 0; i < months; i++)
            {
                decimal monthlyRate = rates[Math.Min(i, rates.Count - 1)] / 100 / 12;
                int remainingMonths = months - i;
                decimal monthlyPayment = CalculateMonthlyPayment(balance, monthlyRate, remainingMonths);
